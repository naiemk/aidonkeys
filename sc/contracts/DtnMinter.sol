...
import "@deeptrust/contracts/contracts/with-dtn-ai.sol";
import "@deeptrust/contracts/contracts/dtn-defaults.sol";
...
contract DtnMinter is WithDtnAI, Ownable, IDtnMinter {
    ...
    public string imageModel = "model.system.openai-gpt-o3-simpleimage";
    public string textModel = "model.system.openai-gpt-o3-simpletext";
    public string server = "node.tester.node1";
    public uint feePerByte = 0.000001 ** 10**18;

    event ArtRequest(uint64 indexed purchaseId, bytes32 artRequestId, string text);
    event NftRequest(uint64 indexed purchaseId, bytes32 nftRequestId, bytes32 artRequestId, string cid);
    ...
    function setAi(address _dtnRouter) public onlyOwner {
        setAi(_dtnRouter);
    }
    ...
    function mintRequest(
        ...
        emit ArtRequest(purchaseId, requestId, text);
    }
    ...
    function artCreatedCallback(bytes32 requestId) external onlyDtn {
        ...
        bytes memory extraParamsEncoded = abi.encode(
            cid, // {0}
            artRequests[requestId].eraId, // {1:uint64}
            artRequests[requestId].artist, // {2:address}
            artRequests[requestId].artistId, // {3}
            artRequests[requestId].text, // {4}
            artRequests[requestId].mintPrice // {5:uint}
        );
        ...
        emit NftRequest(purchaseId, nftRequestId, requestId, cid);
    }
    ...
    function nftCreatedCallback(bytes32 requestId) external onlyDtn {
        ...
        (, , bytes memory response) = ai.fetchResponse(_requestId);
        string memory cid = abi.decode(response, (string)); // Image CID
        IEraNft(nft).mintTokenForPurchase(nftRequests[requestId].purchaseId, Strings.concat(gatewayBase, cid));
        emit NftCreated(nftRequests[requestId].purchaseId, requestId, cid);
    }
    ...
    function aiErrorArt(bytes32 requestId) external onlyDtn {
        ...
        uint purchaseId = artRequests[requestId].purchaseId;
    ...
    function aiErrorNft(bytes32 requestId) external onlyDtn {
        ...
        // Reject the NFT creation request and refund the payment
        (, string memory message, ) = ai.fetchResponse(_requestId);
        uint purchaseId = artRequests[nftRequest[requestId]].purchaseId;
        emit NftError(purchaseId, requestId, nftRequests[requestId], message);
    }

    function addEra(uint64 eraId, string memory eraPrompts) external override {
        require(msg.sender == nft, "Only nft can add era");
        ai.addEra(eraId, eraPrompts);
    }
